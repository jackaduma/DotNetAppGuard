...
import mk.DotNetApp.PE.BoundImport;
import mk.DotNetApp.PE.BoundImportDirectoryTable;
import mk.DotNetApp.PE.COFFHeader;
import mk.DotNetApp.PE.DOSHeader;
import mk.DotNetApp.PE.DOSStub;
import mk.DotNetApp.PE.DebugDirectory;
import mk.DotNetApp.PE.ExportDirectory;
import mk.DotNetApp.PE.ImageData;
import mk.DotNetApp.PE.ImageDataDirectory;
import mk.DotNetApp.PE.ImportDirectory;
import mk.DotNetApp.PE.ImportDirectoryEntry;
import mk.DotNetApp.PE.ImportDirectoryTable;
import mk.DotNetApp.PE.ImportEntry;
import mk.DotNetApp.PE.LoadConfigDirectory;
import mk.DotNetApp.PE.OptionalHeader;
import mk.DotNetApp.PE.PE;
import mk.DotNetApp.PE.PESignature;
import mk.DotNetApp.PE.RVAConverter;
import mk.DotNetApp.PE.ResourceDirectory;
import mk.DotNetApp.PE.ResourceDirectoryTable;
import mk.DotNetApp.PE.ResourceEntry;
import mk.DotNetApp.PE.SectionData;
import mk.DotNetApp.PE.SectionHeader;
import mk.DotNetApp.PE.SectionTable;
import mk.DotNetApp.PE.CLI.CLRHeader;
import mk.DotNetApp.PE.CLI.MetaDataHeader;
import mk.DotNetApp.PE.constant.ImageDataDirectoryType;
import mk.DotNetApp.PE.util.IntMap;
...
public class PEParser
{
    ...
    public static PE read(IDataReader dr) throws IOException {
        ...
        while ((entry = findNextEntry(pe, dr.getPosition())) != null) {
            if (entry.isSection) {
                ...
                System.out.println("This is a DebugRawData");
            } else {
    ...
    private static void readImageData(PE pe, DataEntry entry, IDataReader dr)
        ...
        case ImageDataDirectoryType.CLR_RUNTIME_HEADER:
        	...
        	CLRHeader clrHeader = readCLRHeader(b);
        	id.setClrRuntimeHeader(clrHeader);            
            break;
    ...
    /*
     * add for .NET file format
     * @author: kun ma 
     */
    private static CLRHeader readCLRHeader(byte[] bData) throws IOException {
    	CLRHeader clrHeader = new CLRHeader();
    	ByteArrayDataReader badr = new ByteArrayDataReader(bData);
    	
    	clrHeader.setData(bData);
    	
    	clrHeader.setSize(badr.readDoubleWord());
    	clrHeader.setMajorRuntimeVersion(badr.readWord());
    	clrHeader.setMinorRuntimeVersion(badr.readWord());
    	ImageDataDirectory metadata = new ImageDataDirectory();
    	metadata.setVirtualAddress(badr.readDoubleWord());
        metadata.setSize(badr.readDoubleWord());
    	clrHeader.setMetaData(metadata);
    	clrHeader.setFlags(badr.readWord());
    	clrHeader.setEntryPointToken(badr.readWord());
    	ImageDataDirectory resources = new ImageDataDirectory();
    	resources.setVirtualAddress(badr.readDoubleWord());
        resources.setSize(badr.readDoubleWord());
    	clrHeader.setResources(resources);
    	ImageDataDirectory strongNameSignature = new ImageDataDirectory();
    	strongNameSignature.setVirtualAddress(badr.readDoubleWord());
        strongNameSignature.setSize(badr.readDoubleWord());
    	clrHeader.setStrongNameSignature(strongNameSignature);
    	ImageDataDirectory codeManagerTable = new ImageDataDirectory();
    	codeManagerTable.setVirtualAddress(badr.readDoubleWord());
        codeManagerTable.setSize(badr.readDoubleWord());
    	clrHeader.setCodeManagerTable(codeManagerTable);
    	ImageDataDirectory vTableFixups = new ImageDataDirectory();
    	vTableFixups.setVirtualAddress(badr.readDoubleWord());
        vTableFixups.setSize(badr.readDoubleWord());
    	clrHeader.setVTableFixups(vTableFixups);
    	ImageDataDirectory exportAddressTableJumps = new ImageDataDirectory();
    	exportAddressTableJumps.setVirtualAddress(badr.readDoubleWord());
        exportAddressTableJumps.setSize(badr.readDoubleWord());
    	clrHeader.setExportAddressTableJumps(exportAddressTableJumps);
    	ImageDataDirectory managedNativeHeader = new ImageDataDirectory();
    	managedNativeHeader.setVirtualAddress(badr.readDoubleWord());
    	managedNativeHeader.setSize(badr.readDoubleWord());
    	clrHeader.setManagedNativeHeader(managedNativeHeader);
